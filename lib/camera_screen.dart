...
class _CameraScreenState extends State<CameraScreen> with SingleTickerProviderStateMixin {
  ...
  void _showResult(Face face) {
    final faceAgeEstimate = 35 +
        (face.smilingProbability != null && face.smilingProbability! > 0.5 ? -2 : 0);

    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => HealthForm(faceAgeEstimate: faceAgeEstimate),
      ),
    );
  }
    ...
    return Scaffold(
      body: Stack(
        ...
        children: [
          ...
          // 🎯 Статичная рамка (режим ожидания)
          if (!_isScanning)
            Positioned.fill(
              child: Center(
                child: CustomPaint(
          ...
          // 📊 Оверлей сканирования + компактные логи терминов
          if (_isScanning)
            AnimatedBuilder(
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  color: Colors.black54, borderRadius: BorderRadius.circular(12.0)),
              child: Text(
                _status,
          ...
          if (!_isProcessing)
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    color: Colors.black38, borderRadius: BorderRadius.circular(10.0)),
                child: const Text(
...
// 🎨 Холст рамки (толщина 6 зафиксирована)
class CenteredScannerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isScanning) {
      ...
      canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(20.0)), glowPaint);

      final scanY = rect.top + (size.height * scanProgress);
      const scanHeight = 8.0;
      final scanGradient = LinearGradient(
        colors: [const Color(0x0000D4AA), const Color(0xFF00D4AA), const Color(0x0000D4AA)],
...
// 📊 Компактный лог терминов (высота ~130px, по центру экрана)
...
class _TermLogState extends State<_TermLog> {
  ...
  Widget build(BuildContext context) {
    ...
    final displayItems = _visible.take(3).toList();
    final isLeft = widget.side == 'left';

    return Align(
      alignment: isLeft ? Alignment.centerLeft : Alignment.centerRight,
      child: Container(
        height: 130, // ✅ Уменьшено ~в 4 раза
        width: 240,
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center, // Центрируем по вертикали внутри коробки
          children: List.generate(displayItems.length, (i) {
            final opacity = i == 0 ? 1.0 : 0.4;
            final fontSize = i == 0 ? 18.0 : 14.0;
            return Padding(
